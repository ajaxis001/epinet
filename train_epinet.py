...
optimizer='adam'
batch_size = 5
epochs = 1 # this is how many times the batch of patches of a single image will be sent through the network
verbose_in_fit = 1
...
# init path names to display outside loop
save_weights_full_path = ''
log_full_path = ''

batch_epochs = 30 # Number of times all the image batches in the training data will be seen by the network 
real_epochs = number_of_batches * 30 # total number of forward and backward passes on a single image (NOTE : not patches of the image) 

for idx_b in range(batch_epochs):
    print('\n\nBatch Epoch Number : ', str(idx_b) )

    if idx_b > 0: 
        # If not the first epoch load the weights from the last batch of previous epoch 
        load_weights_name =   'model_weights_idx_'+ str(number_of_batches-1) + '_bepoch_' + str(idx_b-1)  +'.h5'
    ...
    for idx in range(number_of_batches): 
        ...
        if idx > 0 :
            load_weights_name =   'model_weights_idx_'+ str(idx-1) + '_bepoch_' + str(idx_b)  +'.h5'
            load_weights_full_path = os.path.join(dirname,
                                                  models_folder,
                                                  suite_dirname,
                                                  save_weights_to_path,
        ...
        log_name = 'model_idx_'+ str(idx) + '_bepoch_' + str(idx_b) + '.log'
        ...
        save_weights_name =   'model_weights_idx_'+ str(idx)  + '_bepoch_' + str(idx_b) + '.h5'
